...
def detect(opt):
    ...
    if save_txt or save_vid:
        ...
        # Ensure all video writers are properly released
        for writer in vid_writer:
            if isinstance(writer, cv2.VideoWriter):
                writer.release()
        dest = '/home/ubuntu/output/'
        shutil.copy(save_path, dest)
    if update:
        strip_optimizer(yolo_model)  # update model (to fix SourceChangeWarning)


def conv_cards_to_classes(c):
    classes={}
    classes["classes"]={}
    classes["classes"]["low"] = 0
    classes["classes"]["none"] = 0
    classes["classes"]["high"] = 0
    for key in c:
        if key[0] in ['2', '3', '4', '5', '6']:
            classes["classes"]["low"] += 1
        elif key[0] in ['7', '8', '9']:
            classes["classes"]["none"] += 1
        else:
            classes["classes"]["high"] += 1

    import json
    counts_file = os.path.join('/home/ubuntu/output', "class_counts.json")
    try:
        with open(counts_file, 'w') as f:
            json.dump(classes, f, indent=2)
            print(f"\nClass counts saved to: {counts_file}")
    except Exception as e:
        print(f"Could not save class counts: {e}")
    return classes
...
if __name__ == '__main__':
    ...
    print(cards)
    classes = conv_cards_to_classes(cards)
    print(classes)
