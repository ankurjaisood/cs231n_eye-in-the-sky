...
def create_baseline_predictions(ground_truth_data):
    """Create baseline predictions where no cards are counted (all zeros)"""
    baseline_predictions = {}

    for video in ground_truth_data.keys():
        # Baseline: predict 0 cards for all classes
        baseline_predictions[video] = {
            'low': 0,
            'high': 0,
            'none': 0
        }
...
def evaluate_baseline(ground_truth_data, baseline_predictions):
    """Evaluate baseline predictions against ground truth"""
    ...
    print("="*80)
    print("BASELINE EVALUATION: NO CARDS COUNTED")
    print("="*80)
    print("This baseline represents the worst-case scenario where no cards")
    print("are detected/counted at all (all predictions = 0).")
    print("="*80)

    for video in sorted(ground_truth_data.keys(), key=lambda x: int(x.split('.')[0])):
        # Extract ground truth
        gt_data = ground_truth_data[video]
        if 'classes' in gt_data:
            gt_counts = gt_data['classes']
        else:
            gt_counts = gt_data

        # Get baseline predictions (all zeros)
        pred_counts = baseline_predictions[video]
        ...
        pred_hilo = calculate_hilo_count(pred_counts)  # Always 0 for baseline
...
def print_baseline_summary(total_fp, total_fn, total_tp, hilo_comparisons):
    ...
    # Calculate total cards in ground truth
    total_gt_cards = total_tp_all + total_fn_all

    # Calculate additional error metrics
    total_errors = total_fp_all + total_fn_all
    mean_errors_per_video = total_errors / total_videos if total_videos > 0 else 0

    print(f"\nERROR ANALYSIS SUMMARY:")
    print("-" * 40)
    print(f"Mean Absolute Hi-Lo Error: {mean_hilo_error:.2f}")
    print(f"Maximum Hi-Lo Error: {max_hilo_error}")
    print(f"Mean Errors per Video: {mean_errors_per_video:.2f}")
    print(f"Total Detection Errors: {total_errors}")

    print(f"\nBASELINE INSIGHTS:")
    print("-" * 40)
    print(f"Total cards in ground truth: {total_gt_cards}")
    print(f"Total cards missed by baseline: {total_fn_all} (100.0%)")
    print(f"This baseline shows the worst possible performance when no cards are detected.")
    print(f"Any functional card counting system should significantly outperform these metrics.")

def save_baseline_results(video_results, hilo_comparisons, baseline_predictions, output_file):
    ...
    baseline_results = {
        ...
        'metadata': {
            ...
            'baseline_type': 'no_counting',
            'description': 'Baseline where no cards are counted (all predictions = 0)'
        }
    }
...
def main():
    parser = argparse.ArgumentParser(description='Create baseline results for card counting evaluation (no cards counted)')
    parser.add_argument('ground_truth', help='Path to ground truth JSON file (clips_dataset.json)')
    parser.add_argument('--output', '-o', help='Save baseline results to JSON file')
    parser.add_argument('--save-predictions', help='Save baseline predictions to JSON file for use with evaluate_predictions.py')
    ...
    baseline_predictions = create_baseline_predictions(ground_truth_data)

    # Evaluate baseline
    video_results, hilo_comparisons, total_fp, total_fn, total_tp = evaluate_baseline(
        ground_truth_data, baseline_predictions
    )

    # Print summary
    print_baseline_summary(total_fp, total_fn, total_tp, hilo_comparisons)

    # Save results if requested
    if args.output:
        save_baseline_results(video_results, hilo_comparisons, baseline_predictions, args.output)

    # Save predictions for use with existing evaluation script
    if args.save_predictions:
        with open(args.save_predictions, 'w') as f:
            json.dump(baseline_predictions, f, indent=2)
        print(f"\nBaseline predictions saved to: {args.save_predictions}")
        print(f"You can now run: python utils/evaluate_predictions.py {args.ground_truth} {args.save_predictions}")
